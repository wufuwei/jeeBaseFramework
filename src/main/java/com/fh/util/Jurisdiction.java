...
public class Jurisdiction {
	...
	public static boolean readMenu(List<Menu> menuList,String menuUrl,Session session,String USERNAME){
		System.out.println("readMenu(List<Menu> menuList="+menuList+",String menuUrl="+menuUrl+",Session session="+session+",String USERNAME="+USERNAME+")");
		String requestMenuUrlSpit=menuUrl.split(".do")[0];
		String ownerMenu=null;
		for(int i=0;i<menuList.size();i++){
			
			System.out.println("menuList.get("+i+")="+menuList.get(i));
			ownerMenu=menuList.get(i).getMENU_URL().split(".do")[0];
			System.out.println("ownerMenu="+ownerMenu+"   requestMenuUrlSpit="+requestMenuUrlSpit);
			if(ownerMenu.equals(requestMenuUrlSpit)){ //访问地址与菜单地址循环匹配，如何匹配到就进一步验证，如果没匹配到就不处理(可能是接口链接或其它链接)
